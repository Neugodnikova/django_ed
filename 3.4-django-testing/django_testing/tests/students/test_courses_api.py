...
def test_get_courses_list(api_client, course_factory):
    ...
    course1 = course_factory()  # Создаем первый курс
    course2 = course_factory()  # Создаем второй курс
...
def test_filter_courses_by_id(api_client, course_factory):
    ...
    course1 = course_factory()  # Создаем курс с уникальным ID
    course2 = course_factory()  # Создаем второй курс
    url = reverse('courses-list') + f'?id={course1.id}'  # Строим URL с фильтром по ID
    response = api_client.get(url)  # Делаем GET-запрос с фильтром

    assert response.status_code == status.HTTP_200_OK  # Проверяем успешный ответ
    assert len(response.data) == 1  # Проверяем, что вернулся только один курс
...
def test_filter_courses_by_name(api_client, course_factory):
    """Тест для фильтрации курсов по имени"""
    course1 = course_factory(name="Course 1")  # Создаем курс с именем "Course 1"
    course2 = course_factory(name="Course 2")  # Создаем курс с именем "Course 2"
    url = reverse('courses-list') + f'?name=Course 1'  # Строим URL с фильтром по имени
    response = api_client.get(url)  # Делаем GET-запрос с фильтром
...
def test_update_course(api_client, course_factory):
    ...
    course = course_factory(name="Old Name")  # Создаем курс с старым именем
